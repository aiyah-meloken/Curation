...
function statusBadge(s: string) {
  ...
  return <span style={{ color: colors[s] ?? "var(--text-muted)", fontSize: "var(--fs-sm)" }}>{s}</span>;
}

function logLevelColor(type: string) {
  if (type.includes("stage_start") || type === "STAGE") return "var(--accent-gold)";
  if (type.includes("done") || type === "DONE") return "var(--accent-green)";
  if (type.includes("fail") || type === "ERROR" || type === "failed") return "var(--accent-red)";
    ...
    <div style={{ padding: 16 }}>
      <div style={{ background: "var(--bg-panel)", borderRadius: 8, padding: 12, marginBottom: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: "var(--fs-sm)" }}>
          ...
          {run.error_msg && <div style={{ gridColumn: "1/3" }}><span style={{ color: "var(--text-muted)" }}>错误：</span><span style={{ color: "var(--accent-red)" }}>{run.error_msg}</span></div>}
        </div>
      </div>
...
function StreamLogTab({ runId }: { runId: number }) {
  ...
  if (lines.length === 0) return <div style={{ padding: 20, color: "var(--text-muted)" }}>暂无日志</div>;
...
function FileListTab({ runId }: { runId: number }) {
  ...
  if (isLoading) return <div style={{ padding: 20, color: "var(--text-muted)" }}>加载中...</div>;
  if (!files || files.length === 0) return <div style={{ padding: 20, color: "var(--text-muted)" }}>暂无文件</div>;
    ...
    <div style={{ padding: 16 }}>
      {files.map((f) => (
        <div key={f}
          ...
          style={{ background: "var(--bg-panel)", borderRadius: 6, padding: "6px 12px", marginBottom: 4, cursor: "pointer" }}>
          <span style={{ color: "var(--accent-blue)", fontSize: "var(--fs-sm)" }}>{f}</span>
        </div>
      ))}
      {viewingFile && fileContent && (
        <pre style={{ background: "var(--bg-panel)", borderRadius: 8, padding: 12, marginTop: 12, fontFamily: "monospace", fontSize: "var(--fs-xs)", color: "var(--text-secondary)", overflow: "auto", whiteSpace: "pre-wrap", maxHeight: 400 }}>
          {fileContent}
        </pre>
      )}
    </div>
      ...
      <div style={{
          ...
          {(["overview", "stream", "files"] as const).map((t) => {
            ...
            return (
              <button key={t} onClick={() => setTab(t)}
                style={{ background: "none", border: "none", color: tab === t ? "var(--text-primary)" : "var(--text-muted)", padding: "8px 12px", cursor: "pointer", fontSize: "var(--fs-sm)", borderBottom: tab === t ? "2px solid var(--accent-blue)" : "2px solid transparent" }}>
                {labels[t]}
              </button>
            );
          })}
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {run ? (
            ...
            <div style={{ padding: 20, color: "var(--text-muted)" }}>加载中...</div>
          )}
        </div>
      </div>
    </>
