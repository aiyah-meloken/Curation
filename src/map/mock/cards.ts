...
// Minimal topic stub map (id → TopicRef) so mock data type-checks.
// Labels are placeholders; real labels come from the server in production.
const MOCK_TOPICS: Record<string, TopicRef> = {
...
const mk = (
  partial: Pick<MapCard, "card_id" | "article_id" | "title" | "description"> & {
    account: string;
    publish_time: string;
...
export const mockAtlasCards: MapCard[] = [
  // ===== AI 模型 / 预训练 =====
  mk({
    card_id: "c1",
