...
import { AtlasTablePage } from "./components/AtlasTablePage";
...
function deriveDsl(cards: typeof taggedCards, perspective: PerspectiveKey): AtlasDSL {
  // IMPORTANT: only use tags[0] (the card's primary classification).
  // `convertToAtlasCards` routes each card to its tags[0] L2, so if we
...
function convertToAtlasCards(
  ...
  return cards.map((tc) => ({
    ...
    // AtlasCard = InboxItem, so entities is the connection-line source.
...
export function AtlasPreviewApp() {
  ...
  const [theme, setTheme] = useState<AtlasTheme>(loadTheme);
  useEffect(() => {
  ...
  const setHoveredStore = useAtlasStore((s) => s.setHoveredCard);
  const closeDrawerStore = useAtlasStore((s) => s.closeDrawer);
  useEffect(() => {
  ...
  // Override layer
  const [overrides, setOverridesState] = useState<AtlasOverrides>(() =>
    loadOverrides(),
  );
    ...
    >
      <span
        style={{
          ...
          borderRight: "1px solid var(--atlas-ink-2)",
          marginRight: 4,
        }}
      >
        Atlas
        ...
        return (
          <button
            ...
            style={{
              ...
              background: i === activeIdx ? "var(--atlas-ink)" : "transparent",
              color: i === activeIdx ? "var(--atlas-vellum)" : "var(--atlas-ink)",
              border: "none",
              padding: "8px 12px",
              cursor: "pointer",
              transition: "all .15s",
      ...
      <button
        ...
        style={{
          ...
          color: "var(--atlas-ink)",
          border: "none",
          borderLeft: "1px solid var(--atlas-ink-2)",
          padding: "8px 12px",
          marginLeft: 4,
          cursor: "pointer",
          lineHeight: 1,
